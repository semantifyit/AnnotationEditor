...
router.get('/:webApiPath/:actionPath', async (req, res) => {
  ...
  preProcessWebAPI(webAPI);
  const result = webAPI.actions.find(
    ({ path }) => path === req.params.actionPath,
  );
  if (!result) {
...
router.post('/:webApiPath/:actionPath', async (req, res) => {
  try {
    ...
    if (!webAPI) {
      res
        .status(404)
        .json({ err: `WebAPI ${req.params.webApiPath} not found` });
      return;
    }
    preProcessWebAPI(webAPI);
    const action = webAPI.actions.find(
      ({ path }) => path === req.params.actionPath,
    );
    if (!action) {
      res
        .status(404)
        .json({ err: `Action ${req.params.actionPath} not found` });
      return;
    }
