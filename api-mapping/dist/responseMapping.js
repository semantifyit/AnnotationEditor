    ...
    if (Array.isArray(mappingObj) && Array.isArray(input)) {
        if (mappingObj.length === 1) {
            ...
            input.forEach(function (inputElem, i) {
                var _a;
                return doMapping(mappingObj[0], inputElem, result, iterator_1 ? Object.assign(iterators, (_a = {}, _a[iterator_1] = i, _a)) : iterators, options);
            });
        }
        else {
            ...
            if (typeof value === 'string' && value.trim().startsWith('$')) {
                if (key === '_set') {
                    ...
                    setVals.forEach(function (setVal) {
                        var _a = setVal.split('='), setValPath = _a[0], setValVal = _a[1];
                    ...
                    var _b = util_1.parsePathStr(value, true), path = _b.path, transformFunction = _b.transformFunction;
                    var iteratorPath = util_1.replaceIterators(path, iterators);
                    if (options.evalMethod && transformFunction) {
                        var transformedValue = util_1.transFormValue(input[key], transformFunction, options.evalMethod);
...
exports.responseMapping = function (userInputResponse, userMapping, userOptions, mergeObj) {
    if (userOptions === void 0) { userOptions = defaultResponseOptions; }
    return __awaiter(_this, void 0, void 0, function () {
        var input, mapping, result, options, _a, _b, rmlStr, rmlResult, e_1;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    if (!(options.type === 'xml' && mapping.body && typeof mapping.body === 'string' && input.body && typeof input.body === 'string')) return [3, 3];
                    _a = mapping;
                ...
                case 3:
                    ...
                    if (!(options.type === 'json' || options.type === 'xml')) return [3, 4];
                    doMapping(mapping, input, result, {}, options);
                    return [3, 7];
                case 4:
                    if (!(options.type === 'yarrrml' && typeof mapping.body === 'string' && typeof input.body === 'string')) return [3, 7];
